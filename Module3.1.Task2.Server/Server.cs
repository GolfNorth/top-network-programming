...
namespace Module3_1.Task2.Server;
...
public sealed class Server(int port)
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        listener.Start();
        Log?.Invoke($"Server started on port {port}. Limit: {MaxQuotesPerClient} quotes per client.");
        try
        {
    ...
    private async Task HandleClientAsync(TcpClient tcpClient, CancellationToken ct)
    {
        ...
        try
        {
            ...
            while ((line = await reader.ReadLineAsync(ct)) is not null)
            {
                ...
                {
                    if (quotesSent.Count >= MaxQuotesPerClient)
                    {
                        await writer.WriteLineAsync("LIMIT_REACHED");
                        Log?.Invoke($"{DateTime.Now:HH:mm:ss} {remote} limit reached");
                        break;
                    }

using System.Net;
using System.Net.Sockets;
using System.Text;
using Module3_2.Common;

namespace Module3_2.Task4.Server;

public sealed class UdpServer(int port)
{
    private const int MaxClients = 10;
    private static readonly TimeSpan InactiveTimeout = TimeSpan.FromMinutes(10);

    private readonly Dictionary<string, DateTime> _clients = new();
    private readonly Lock _lock = new();
    private UdpClient? _udp;

    public event Action<string>? Log;
    public event Action<int>? ClientCountChanged;

    public async Task StartAsync(CancellationToken ct)
    {
        _udp = new UdpClient(port);
        Log?.Invoke($"Server started on port {port}.");
        _ = CleanupLoopAsync(ct);
        while (!ct.IsCancellationRequested)
        {
            var result = await _udp.ReceiveAsync(ct);
            _ = HandleAsync(result);
        }
    }

    public void Stop()
    {
        _udp?.Close();
        Log?.Invoke("Server stopped.");
    }

    private async Task CleanupLoopAsync(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            await Task.Delay(TimeSpan.FromMinutes(1), ct);
            var now = DateTime.Now;
            lock (_lock)
            {
                var expired = _clients
                    .Where(kv => now - kv.Value > InactiveTimeout)
                    .Select(kv => kv.Key).ToList();
                foreach (var ip in expired)
                {
                    _clients.Remove(ip);
                    Log?.Invoke($"Client {ip} timed out.");
                }
                if (expired.Count > 0) ClientCountChanged?.Invoke(_clients.Count);
            }
        }
    }

    private async Task HandleAsync(UdpReceiveResult result)
    {
        if (_udp is null) return;
        var request = Encoding.UTF8.GetString(result.Buffer).Trim();
        var ip = result.RemoteEndPoint.Address.ToString();

        string response;
        lock (_lock)
        {
            var isKnown = _clients.ContainsKey(ip);
            if (!isKnown && _clients.Count >= MaxClients)
            {
                response = "SERVER_FULL";
                Log?.Invoke($"{ip}: rejected (server full)");
                _ = SendAsync(response, result.RemoteEndPoint);
                return;
            }
            _clients[ip] = DateTime.Now;
            if (!isKnown)
            {
                Log?.Invoke($"{ip}: new client (active: {_clients.Count}/{MaxClients})");
                ClientCountChanged?.Invoke(_clients.Count);
            }
        }

        if (request.Equals("BYE", StringComparison.OrdinalIgnoreCase))
        {
            lock (_lock) { _clients.Remove(ip); ClientCountChanged?.Invoke(_clients.Count); }
            response = "BYE";
            Log?.Invoke($"{ip}: disconnected");
        }
        else
        {
            response = Process(request);
            Log?.Invoke($"{ip}: {request} -> {response}");
        }

        await SendAsync(response, result.RemoteEndPoint);
    }

    private async Task SendAsync(string message, IPEndPoint ep)
    {
        if (_udp is null) return;
        var bytes = Encoding.UTF8.GetBytes(message);
        await _udp.SendAsync(bytes, ep);
    }

    private static string Process(string request)
    {
        if (request.Equals("LIST", StringComparison.OrdinalIgnoreCase))
            return string.Join("\n", PartsRepository.Parts.Select(kv => $"{kv.Key}: {kv.Value:N0} rub"));

        if (request.StartsWith("PRICE ", StringComparison.OrdinalIgnoreCase))
        {
            var part = request[6..].Trim();
            var price = PartsRepository.GetPrice(part);
            return price.HasValue ? $"{part}: {price.Value:N0} rub" : $"NOT_FOUND: {part}";
        }

        return "ERROR: use LIST or PRICE <name>";
    }
}

...
namespace Module3_1.Task3.Server;
...
public sealed class Server(int port)
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        listener.Start();
        Log?.Invoke($"Server started on port {port}. Max clients: {MaxClients}.");
        try
        {
    ...
    private async Task HandleClientAsync(TcpClient tcpClient, CancellationToken ct)
    {
        ...
        if (active > MaxClients)
        {
            ...
            Log?.Invoke($"{DateTime.Now:HH:mm:ss} {remote} rejected: server full");
            return;
        }

        var quotesSent = new List<string>();
        Log?.Invoke($"{DateTime.Now:HH:mm:ss} {remote} connected (active: {active}/{MaxClients})");
        try
        {
